...
export default function ListMovements() {
    ...
    const [activeTab, setActiveTab] = useState('ativas'); // aba ativa (ativas/canceladas)
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            Alert.alert('Erro', 'Não foi possível deletar a movimentação. Verifique a conexão ou o servidor.');
                        }
                    }
                }
            ]
...
*/}
    ...
    useEffect(() => {
        if (isFocused) { // Recarrega os dados apenas quando a tela está em foco
            fetchMovements();
        }
        fetchUserProfile();
    }, [isFocused]);
        ...
        return (
            <View style={styles.card}>
                <View style={styles.cardContent}>
                    {/* botão para navegar para detalhes da movimentação, visível apenas para usuários que não são "filial" */}
                    {userProfile !== 'filial' && (
                        <TouchableOpacity
                    ...
                    {/* picker de status visível apenas para usuários que não são do tipo "filial" */}
                    {userProfile !== 'filial' && (
                    ...
                    {/* botão para deletar a movimentação */}
                    <TouchableOpacity
